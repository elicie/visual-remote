    ...
    const handleMessage = (data: RawData) => {
      ...
      try {
        if (message.type === "browser.hello") {
          const previousSession = controlSessions.get(socket);
          if (previousSession && previousSession !== message.browserSessionId) throw new ControlServiceError(400, "session_mismatch", "Control socket cannot change browser sessions");
          controlSessions.set(socket, message.browserSessionId);
