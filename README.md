...
2. `현재 탭 공유`를 누르고 Chrome 공유 창에서 **현재 탭**을 선택합니다.
3. 요청을 보내면 기존 CLI의 Figma MCP로 기준 PNG와 구조 데이터를 확보합니다.
4. 기준 파일을 고정한 뒤 기존 프로젝트를 수정하고, 공유된 실제 화면을 캡처합니다.
5. 픽셀·영역별 점수와 텍스트 위치·크기·스타일 차이를 계산하여 필요하면 재수정합니다.
6. 작업 보드에서 기준 이미지, 현재 캡처, 히트맵, 겹쳐 보기와 각 회차의 결과를 확인합니다.

캡처에는 현재 탭을 확인할 수 있는 Capture Handle 지원 데스크톱 Chrome이 필요합니다.
창이나 전체 화면 공유는 사용하지 않습니다. 로그인 정보를 복사하거나 별도 브라우저를
띄우지 않으며, 캡처 순간에는 Overlay를 숨깁니다. 탭 공유 중지·페이지 이동·뷰포트 변경·
캡처 영역과 Figma 크기 불일치는 이유를 표시하고 비교를 중단합니다. 페이지 모드는 전체
스크롤 문서가 아니라 현재 뷰포트이며, 크기를 억지로 늘이거나 줄여 일치시키지 않습니다.
